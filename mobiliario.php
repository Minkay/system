...
if(!isset($buscar)){
	...
	$resultados = $sel->paginadorMobiliario($inicio,$registros);  
	$total_registros = mysqli_num_rows($ni); 
	$total_paginas = ceil($total_registros / $registros);
}else {
	$ni = $sel->consultaMobiliarioCon($buscar); 
	$registros = 20;
	$contador = 1;
	$pagina = $_GET["pagina"];
	...
	$resultados = $sel->paginadorMobiliarioCon($buscar,$inicio,$registros);  
	$total_registros = mysqli_num_rows($ni); 
	$total_paginas = ceil($total_registros / $registros);
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
             <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Mobiliario<small>Apertura de registro</small></h2>
               
                    <form class="form-inline" style="text-align: right;" action="mobiliario.php" method="POST">
                      ...
                      if ($total_registros) {
                       while($mue = $resultados->fetch_assoc()){ ?>
                          <tr class="odd pointer">
                            ...
                            <td class=" "><?php echo $mue["id_mob"]; ?></td>
						  ...
						  <?php if($mue["declara"]=="SI"){
				                              ...
				                              echo " <a href='DetalleMobiliario.php?id=".$mue["id_mob"]."&code=".$mue["code"]. "' class='btn btn-danger btn-xs'>Editar</a>";
				                                }?></td>
						  <!-- <a href="DetalleMobiliario.php?cod=<?php echo $mue["id_mob"]; ?>" class="btn btn-danger btn-xs">Modificar</a> -->
                          </tr>
                          <?php }
                                   } 
                               ...
                               <ul class="pagination">
                                  ...
                                  <?php
                                      if ($total_registros) {
                                        ...
                                        if (($pagina - 1) > 0) {
                                          echo "<li ><a href='mobiliario.php?pagina=".($pagina-1)."'>< Anterior</a></li>";
                                          } else {
                                        ...
                                        for ($i = 1; $i <= $total_paginas; $i++) {
                                          if ($pagina == $i) {
                                            ...
                                            echo "<li ><a href='mobiliario.php?pagina=$i'>$i</a> </li>"; 
                                          } 
                                        }
                                        if (($pagina + 1)<=$total_paginas) {
                                          echo "<li ><a href='mobiliario.php?pagina=".($pagina+1)."'>Siguiente ></a></li>";
                                        } else {
