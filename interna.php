...
$buscar = $_POST["buscar"];
 ...
 if(!isset($buscar)){
		...
		$resultados = $sel->paginadorInterna($inicio,$registros);  
		$total_registros = mysqli_num_rows($ni); 
		$total_paginas = ceil($total_registros / $registros);
 }else{
 		$ni = $sel->consultaInternaCon($buscar); 
		$registros = 20;
		$contador = 1;
		$pagina = $_GET["pagina"];
		if (!$pagina) { 
		...
		$resultados = $sel->paginadorInternaCon($buscar,$inicio,$registros);  
		$total_registros = mysqli_num_rows($ni); 
		$total_paginas = ceil($total_registros / $registros);
 }
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
             <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Infraestructura Interna<small>Apertura de registro</small></h2>
               
                    <form class="form-inline" style="text-align: right;" action="interna.php" method="POST">
                       ...
                       while($mue = $resultados->fetch_assoc()){ ?>
                          <tr class="odd pointer">
                            ...
                            <td class=" "><?php echo $mue["titulo"]; ?></td>
                             <td class=" "><?php echo $mue["variable"]; ?></td>
						  ...
						  <?php if($mue["declara"]=="SI"){
				                              ...
				                              echo " <a href='DetalleInterna.php?id=".$mue["id_inter"]."&code=".$mue["code"]. "' class='btn btn-danger btn-xs'>Editar</a>";
				                                }?></td>
						  <!-- <a href="DetalleInterna.php?cod=<?php echo $mue["id_inter"]; ?>" class="btn btn-danger btn-xs">Modificar</a>-->
                          </tr>
                          <?php }
                                   } 
                               ...
                               <ul class="pagination">
                                  ...
                                  <?php
                                      if ($total_registros) {
                                        ...
                                        if (($pagina - 1) > 0) {
                                          echo "<li ><a href='interna.php?pagina=".($pagina-1)."'>< Anterior</a></li>";
                                          } else {
                                        ...
                                        for ($i = 1; $i <= $total_paginas; $i++) {
                                          if ($pagina == $i) {
                                            ...
                                            echo "<li ><a href='interna.php?pagina=$i'>$i</a> </li>"; 
                                          } 
                                        }
                                        if (($pagina + 1)<=$total_paginas) {
                                          echo "<li ><a href='interna.php?pagina=".($pagina+1)."'>Siguiente ></a></li>";
                                        } else {
