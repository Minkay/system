...
$buscar = $_POST["buscar"];
 ...
 if(!isset($buscar)){
	...
	$resultados = $sel->paginadorEquipo($inicio,$registros);  
	$total_registros = mysqli_num_rows($ni); 
	$total_paginas = ceil($total_registros / $registros);
 }else{
 	$ni = $sel->consultaEquipoCon($buscar); 
	$registros = 20;
	$contador = 1;
	$pagina = $_GET["pagina"];
	...
	$resultados = $sel->paginadorEquipoCon($buscar,$inicio,$registros);  
	$total_registros = mysqli_num_rows($ni); 
	$total_paginas = ceil($total_registros / $registros);
 }
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
             <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Equipamiento<small>Apertura de registro</small></h2>
               
                     <form class="form-inline" style="text-align: right;" action="equipo.php" method="POST">
                      ...
                      if ($total_registros) {
                       while($mue = $resultados->fetch_assoc()){ ?>
                          <tr class="odd pointer">
                            ...
                            <td class=" "><?php echo $mue["id_eq"]; ?></td>
						  ...
						  <?php if($mue["declara"]=="SI"){
			                              ...
			                              echo " <a href='DetalleEquipo.php?id=".$mue["id_eq"]."&code=".$mue["code"]. "' class='btn btn-danger btn-xs'>Modificar</a>";
			                                }?>
						<!--  <a href="DetalleEquipo.php?cod=<?php echo $mue["id_eq"]; ?>" class="btn btn-danger btn-xs">Modificar</a>-->
                               ...
                               <ul class="pagination">
                                  ...
                                  <?php
                                      if ($total_registros) {
                                        ...
                                        if (($pagina - 1) > 0) {
                                          echo "<li ><a href='equipo.php?pagina=".($pagina-1)."'>< Anterior</a></li>";
                                          } else {
                                        ...
                                        for ($i = 1; $i <= $total_paginas; $i++) {
                                          if ($pagina == $i) {
                                            ...
                                            echo "<li ><a href='equipo.php?pagina=$i'>$i</a> </li>"; 
                                          } 
                                        }
                                        if (($pagina + 1)<=$total_paginas) {
                                          echo "<li ><a href='equipo.php?pagina=".($pagina+1)."'>Siguiente ></a></li>";
                                        } else {
